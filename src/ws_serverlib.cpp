// 
// The following code is a mofified version of Vinnie Falco's advanced_server_flex
// from the Boost Beast.org examples. The code has some minor restructuering to
// provide a WebSocket library that can be used to support a WebSocket server with
// callbacks for sending unsolicited messages to a client. This is used to
// support a TR-369 Controller for sending USP test messages to an USP agent.
//
// Thanks to Vinie.......
...
class websocket_session {
            ...
            [this](websocket::response_type& res) {
              res.set(http::field::server,
                      std::string(BOOST_BEAST_VERSION_STRING) +
                          " falcon-ws-server");
              // add options from library user level.
              for(auto p: *(derived().wsDope()->ws_options_())){
                res.set(p.first, *p.second);  
              }
            }));

    // Accept the websocket handshake
    //   Receives client's Upgrade HTTP request and the server replies with an
    //   HTTP response with result of handshake.
  ...
  on_accept(beast::error_code ec) {
    if (ec)
      return derived().wsDope()->failed_(ec, "accept");
    // Callback to user level to indicate WebSocket session is running.
    if ( derived().wsDope()->ready_) {
      derived().wsDope()->ready_(func_);
    }
    // Read a message
    do_read();
  }
 ...
 public:
  ...
  run(http::request<Body, http::basic_fields<Allocator>> req) {
    ...
    func_ = std::bind(&websocket_session::Send, derived().shared_from_this(),
...
class plain_websocket_session
  ...
  std::shared_ptr<WSDope> ws_dope_;
 public:
  // Create the session
  explicit plain_websocket_session(
      beast::tcp_stream&& stream, std::shared_ptr<WSDope> ws_dope)
      : ws_(std::move(stream)), ws_dope_{ws_dope} {
  }
...
class ssl_websocket_session
 ...
 public:
  ...
  explicit ssl_websocket_session(
      beast::ssl_stream<beast::tcp_stream>&& stream, std::shared_ptr<WSDope> ws_dope)
      : ws_(std::move(stream)), ws_dope_{ws_dope} {
  }
...
void make_websocket_session(
    ...
    http::request<Body, http::basic_fields<Allocator>> req,
    std::shared_ptr<WSDope> ws_dope) {
  std::make_shared<plain_websocket_session>(
      std::move(stream), ws_dope)
      ->run(std::move(req));
}
...
class http_session {
 ...
 public:
  ...
  http_session(
      ...
      : ws_dope_(ws_dope), queue_(*this), buffer_(std::move(buffer)) {
  }
  ...
  on_read(beast::error_code ec, std::size_t bytes_transferred) {
    ...
    if (ec)
      return ws_dope_->failed_(ec, "http read");
    ...
    /*
    // Send the response
    handle_request(std::string{"/"}, parser_->release(), queue_);
  ...
  on_write(bool close, beast::error_code ec, std::size_t bytes_transferred) {
    ...
    if (ec)
      return ws_dope_->failed_(ec, "http write");
...
class ssl_http_session
 ...
 private:
  ...
  on_shutdown(beast::error_code ec) {
    if (ec)
      return ws_dope_->failed_(ec, "shutdown");
...
class detect_session : public std::enable_shared_from_this<detect_session> {
 ...
 public:
  ...
  on_detect(beast::error_code ec, bool result) {
    if (ec)
      return ws_dope_->failed_(ec, "detect");
...
listener::listener(
    ...
    : ioc_(ioc), ctx_(ctx), acceptor_(net::make_strand(ioc)), ws_dope_(ws_dope)
 {
  ...
  if (ec) {
    ws_dope_->failed_(ec, "socket set_option");
    return;
  }
