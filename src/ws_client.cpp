...
// TODO? timer?
void WS_Client::Start( const std::string* host, const std::string* port, const std::string* path){
  host_ = *host;
  path_ = *path;
  resolver_.async_resolve( host_, *port,
   beast::bind_front_handler( &WS_Client::OnResolve, shared_from_this())); 
}

void WS_Client::StartRead(){
  ws_.async_read( buffer_, beast::bind_front_handler(
                &WS_Client::OnRead,
                shared_from_this())); 
}

void WS_Client::OnResolve(const beast::error_code& ec, tcp::resolver::results_type results) {
  if (ec) {
...
void WS_Client::OnConnect(const beast::error_code& ec, tcp::resolver::results_type::endpoint_type ep) {
  PLOG(plog::debug);
  if (ec) {
  ...
  ws_.async_handshake(res_, host_, path_,
                      beast::bind_front_handler(
                          &WS_Client::OnHandshake,
                          shared_from_this()) );

}

void WS_Client::OnHandshake(const beast::error_code &ec){
  if ( ec ){
...
void WS_Client::Write(const unsigned char* msg, const size_t lth){
  ws_.async_write( net::buffer(msg, lth),
        beast::bind_front_handler( &WS_Client::OnWrite, shared_from_this()));
}

void WS_Client::OnWrite(const beast::error_code &ec, std::size_t xfered){
  if ( ec ){
...
void WS_Client::OnRead(const beast::error_code &ec, std::size_t bytesXfered ){
  if ( ec ) {
